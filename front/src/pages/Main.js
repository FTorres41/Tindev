...
export default function Main ({ match }) {
    ...
    return (
        <div className="main-container">
            ...
            </ul>) : (<div className="empty">Não há devs! :(</div>)}

            { matchedDev && (
                <div className="match-container">
                    <img src={itsamatch} alt="It's a match"></img>
                    <img src={matchedDev.avatar} className="avatar" alt={matchedDev.name}></img>
                    <strong>{matchedDev.name}</strong>
                    <p>{matchedDev.bio}</p>
                    <button type="button" onClick={() => setMatchedDev(null)}>FECHAR</button>
                </div>
            )}
        </div>
    )
